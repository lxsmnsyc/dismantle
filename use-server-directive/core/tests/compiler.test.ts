...
describe.each(['server', 'client'] as const)('compile (%s)', (mode) => {
  it(`registers blocks through use-server-directive/${mode}`, async () => {
    const { output, entry, id } = await getEntry(mode, CODE);
    expect(entry).toContain(`from "use-server-directive/${mode}"`);
    expect(output.code).toContain('from "use-server-directive/runtime"');
    expect(id).toMatch(/^\/__server\/[0-9a-f]+-run$/);
    expect(output.roots).toHaveLength(mode === 'server' ? 1 : 0);
  });

  it('uses a custom directive and prefix', async () => {
    const { id } = await getEntry(mode, CODE.replace("'use server'", "'use remote'"), {
