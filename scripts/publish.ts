...
/** A workspace project as listed by `pnpm ls --json`. */
interface WorkspaceProject {
  path: string;
}

function isWorkspaceProjectList(value: unknown): value is WorkspaceProject[] {
  return (
    Array.isArray(value) &&
    value.every(
      (item: unknown) =>
...
/** Returns the directory of every workspace project except the root. */
function listWorkspaceDirs(): string[] {
  const output = execFileSync('pnpm', ['ls', '--recursive', '--json', '--depth', '-1'], {
    cwd: ROOT,
    encoding: 'utf8',
  });
  const parsed: unknown = JSON.parse(output);
  if (!isWorkspaceProjectList(parsed)) {
    throw new Error('pnpm ls returned an unexpected shape');
  }
  const root = join(ROOT);
  return parsed.map((project) => project.path).filter((path) => join(path) !== root);
}
...
for (const dir of listWorkspaceDirs()) {
  ...
  const out = mkdtempSync(join(tmpdir(), 'dismantle-pack-'));
  try {
